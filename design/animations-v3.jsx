...
function Stage({
  ...
  React.useEffect(() => {
    ...
    const step = (ts) => {
      ...
      rafRef.current = schedule(step);
    };
    // rAF is suspended while the document is hidden (background tab, hidden
    // host frame). Fall back to a timer tick so the clock still advances.
    let timerId = null;
    const schedule = (fn) => {
      if (timerId) { clearTimeout(timerId); timerId = null; }
      const id = requestAnimationFrame((ts) => { if (timerId) { clearTimeout(timerId); timerId = null; } fn(ts); });
      timerId = setTimeout(() => { timerId = null; cancelAnimationFrame(id); fn(performance.now()); }, 250);
      return id;
    };
    rafRef.current = schedule(step);
    return () => {
