...
/** Share of the run spent black before the mosaic starts resolving. */
const BLANK = 0.42;

/**
 * A reel card that generates the way the terminal footage does. It waits as a
 * hatched slot, turns black behind its posted date and a progress bar, then
 * the post arrives as a coarse mosaic that sharpens step by step and
 * crossfades into the finished image. Runs once it scrolls into view,
 * staggered by `delay`; reduced motion skips straight to the finished card.
 */
export function ReelCard({
  ...
  delay?: number;
  /** ms from the first black frame to the finished post. */
  duration?: number;
  ...
  return (
    ...
    >
      {/* The posted date, carried through generation and onto the result. */}
      ...
      {/* Generation layer: mosaic canvas and progress. */}
